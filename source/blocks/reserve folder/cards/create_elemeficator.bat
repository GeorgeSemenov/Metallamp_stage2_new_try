...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/cards%filename%.pug
  )> newFile.pug
  type cards.pug >> newFile.pug
  move /Y newFile.pug  cards.pug
  (
...
(
echo @import '%fileName%/cards%fileName%';
)>>cards.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=cards
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=cards%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/cards%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
