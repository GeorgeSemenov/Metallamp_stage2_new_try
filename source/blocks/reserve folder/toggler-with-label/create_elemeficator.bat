@echo off
set parentName=toggler-with-label
...
(
echo include %filename%/toggler-with-label%filename%.pug
)> newFile.pug
type toggler-with-label.pug >> newFile.pug
move /Y newFile.pug  toggler-with-label.pug
(
...
(
echo @import '%fileName%/toggler-with-label%fileName%';
)>>toggler-with-label.scss
cd %fileName%
(
...
(
echo @echo off
echo set grandParentName=toggler-with-label
echo set parentName=%fileName%
echo set /p fileName=What would you create?
echo set newStyleSCSS=toggler-with-label%fileName%.scss
echo mkdir %%fileName%%
echo ^(
echo echo @import '%%fileName%%/toggler-with-label%fileName%%%fileName%%';
echo ^)^>^>%%newStyleSCSS%%
echo cd %%fileName%%
echo ^(
