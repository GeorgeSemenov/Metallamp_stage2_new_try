...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/arrow-toggler%filename%.pug
  )> newFile.pug
  type arrow-toggler.pug >> newFile.pug
  move /Y newFile.pug  arrow-toggler.pug
  (
...
(
echo @import '%fileName%/arrow-toggler%fileName%';
)>>arrow-toggler.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=arrow-toggler
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=arrow-toggler%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/arrow-toggler%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
