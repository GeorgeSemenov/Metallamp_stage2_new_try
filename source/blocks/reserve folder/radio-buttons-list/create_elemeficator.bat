@echo off
set parentName=radio-buttons-list
...
(
echo include %filename%/radio-buttons-list%filename%.pug
)> newFile.pug
type radio-buttons-list.pug >> newFile.pug
move /Y newFile.pug  radio-buttons-list.pug
(
...
(
echo @import '%fileName%/radio-buttons-list%fileName%';
)>>radio-buttons-list.scss
cd %fileName%
(
...
(
echo @echo off
echo set grandParentName=radio-buttons-list
echo set parentName=%fileName%
echo set /p fileName=What would you create?
echo set newStyleSCSS=radio-buttons-list%fileName%.scss
echo mkdir %%fileName%%
echo ^(
echo echo @import '%%fileName%%/radio-buttons-list%fileName%%%fileName%%';
echo ^)^>^>%%newStyleSCSS%%
echo cd %%fileName%%
echo ^(
