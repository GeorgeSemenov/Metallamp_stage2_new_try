...
(
echo include %filename%/checkbox%filename%.pug
)> newFile.pug
type checkbox.pug >> newFile.pug
move /Y newFile.pug  checkbox.pug
(
...
(
echo @import '%fileName%/checkbox%fileName%';
)>>checkbox.scss
cd %fileName%
(
...
(
echo @echo off
echo set grandParentName=checkbox
echo set parentName=%fileName%
echo set /p fileName=What would you create?
echo set newStyleSCSS=checkbox%fileName%.scss
echo mkdir %%fileName%%
echo ^(
echo echo @import '%%fileName%%/checkbox%fileName%%%fileName%%';
echo ^)^>^>%%newStyleSCSS%%
echo cd %%fileName%%
echo ^(
