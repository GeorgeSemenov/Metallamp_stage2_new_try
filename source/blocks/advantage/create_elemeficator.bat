...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/advantage%filename%.pug
  )> newFile.pug
  type advantage.pug >> newFile.pug
  move /Y newFile.pug  advantage.pug
  (
...
(
echo @import '%fileName%/advantage%fileName%';
)>>advantage.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=advantage
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=advantage%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/advantage%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
