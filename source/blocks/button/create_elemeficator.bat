...
(
echo include %filename%/button%filename%.pug
)> newFile.pug
type button.pug >> newFile.pug
move /Y newFile.pug  button.pug
(
...
(
echo @import '%fileName%/button%fileName%';
)>>button.scss
cd %fileName%
(
...
(
echo @echo off
echo set grandParentName=button
echo set parentName=%fileName%
echo set /p fileName=What would you create?
echo set newStyleSCSS=button%fileName%.scss
echo mkdir %%fileName%%
echo ^(
echo echo @import '%%fileName%%/button%fileName%%%fileName%%';
echo ^)^>^>%%newStyleSCSS%%
echo cd %%fileName%%
echo ^(
