set parentName=rate_button-list
...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/rate_button-list%filename%.pug
  )> newFile.pug
  type rate_button-list.pug >> newFile.pug
  move /Y newFile.pug  rate_button-list.pug
  (
...
(
echo @import '%fileName%/rate_button-list%fileName%';
)>>rate_button-list.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=rate_button-list
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=rate_button-list%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/rate_button-list%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
