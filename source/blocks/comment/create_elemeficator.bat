...
(
echo include %filename%/comment%filename%.pug
)> newFile.pug
type comment.pug >> newFile.pug
move /Y newFile.pug  comment.pug
(
...
(
echo @import '%fileName%/comment%fileName%';
)>>comment.scss
cd %fileName%
(
...
(
echo @echo off
echo set grandParentName=comment
echo set parentName=%fileName%
echo set /p fileName=What would you create?
echo set newStyleSCSS=comment%fileName%.scss
echo mkdir %%fileName%%
echo ^(
echo echo @import '%%fileName%%/comment%fileName%%%fileName%%';
echo ^)^>^>%%newStyleSCSS%%
echo cd %%fileName%%
echo ^(
