...
if /I %isThereNeedPugFile% NEQ %zero% (
  ...
  (
  echo include %filename%/advantage-list%filename%.pug
  )> newFile.pug
  type advantage-list.pug >> newFile.pug
  move /Y newFile.pug  advantage-list.pug
  (
...
(
echo @import '%fileName%/advantage-list%fileName%';
)>>advantage-list.scss
cd %fileName
(
...
if  /I %isThereElement% == %True% (
  (
  echo @echo off
  echo set grandParentName=advantage-list
  echo set parentName=%fileName%
  echo set /p fileName=What would you create?
  echo set newStyleSCSS=advantage-list%fileName%.scss
  echo mkdir %%fileName%%
  echo ^(
  echo echo @import '%%fileName%%/advantage-list%fileName%%%fileName%%';
  echo ^)^>^>%%newStyleSCSS%%
  echo cd %%fileName%%
  echo ^(
