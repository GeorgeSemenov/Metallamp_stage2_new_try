...
(
echo include %filename%/root%filename%.pug
)> newFile.pug
type root.pug >> newFile.pug
move /Y newFile.pug  root.pug
(
...
(
echo @import '%fileName%/root%fileName%';
)>>root.scss
cd %fileName%
(
...
(
echo @echo off
echo set grandParentName=root
echo set parentName=%fileName%
echo set /p fileName=What would you create?
echo set newStyleSCSS=root%fileName%.scss
echo mkdir %%fileName%%
echo ^(
echo echo @import '%%fileName%%/root%fileName%%%fileName%%';
echo ^)^>^>%%newStyleSCSS%%
echo cd %%fileName%%
echo ^(
